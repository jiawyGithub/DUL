...
class DULResNet(nn.Module):
    def __init__(self, block, layers, feat_dim = 512, drop_ratio = 0.4, use_se = True, used_as = 'baseline'):
        ...
        self.mu_head = nn.Sequential(
            nn.BatchNorm2d(512 * block.expansion, eps=2e-5, affine=False), # 数据的归一化处理
        ...
        if used_as == 'dul_cls': # use both mu_head and logvar_head
    ...
    def _reparameterize(self, mu, logvar):
        ...
        # torch.randn_like(std): 基于std的相关属性进行0-1随机化; 
    ...
    def forward(self, x):
        ...
        else: # use both mu_head and logvar_head
            mu = self.mu_head(x)
