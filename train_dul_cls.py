...
class DulClsTrainer(mlib.Faster1v1):
    ...
    def _model_loader(self):
        # args.backbone = dulres18; used_as = dul_cls; DULResNet
        ...
        self.model['optimizer'] = torch.optim.SGD(
                                      ...
                                      momentum=0.9,
                                      nesterov=True)
        self.model['scheduler'] = torch.optim.lr_scheduler.MultiStepLR(  # 调整学习率
    ...
    @staticmethod # 可以不需要实例化，直接类名.方法名()来调用。
    ...
    def _train_one_epoch(self, epoch = 0):
        ...
        for idx, (img, gty, _) in enumerate(self.data['train']):
            ...
            loss    = self.model['criterion'](output, gty, mu, logvar) # loss_mode = ce
    ...
    def _dul_training(self):
        ...
        for epoch in range(self.args.start_epoch, self.args.end_epoch + 1):
            ...
            self.model['scheduler'].step() # 调整学习率
